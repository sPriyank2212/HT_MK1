...
/// Destination`. `Source`/`Part Number`/`Destination` stay blank — the wire
/// protocol (`!CONT <hi> <lo> <status>`) carries pins only, never that
/// metadata, and `ConnectorDef` (`design/model.dart`) has no part-number
/// field to draw on either. `Conn ID`/pin label are real once GUI-11's
/// connector mapping resolves the pin (`AppState._onCont`) — `'—'`/the raw
/// flat pin number otherwise (no netlist loaded, or a pin the active
/// fixture doesn't cover), same as before this fix.
class ContReportRow {
  final int testNum;
  final int srcPin;
  final int dstPin;
...
class ContReport {
  ...
  String toCsv() {
    ...
    for (final r in rows) {
      buf.writeln(_csvRow([
        r.testNum, '', '', r.srcConnId, r.srcPinLabel, r.srcPin, r.status,
        r.dstPin, r.dstPinLabel, r.dstConnId, '', '', //
      ]));
    }
    return buf.toString();
  }
...
/// pin label are real once GUI-11's connector mapping resolves the pin — see
/// `ContReportRow`.
class ResReportRow {
  final int testNum;
  final int srcPin;
...
class ResReport {
  ...
  String toCsv() {
    ...
    for (final r in rows) {
      buf.writeln(_csvRow([
        r.testNum, '', '', r.srcConnId, r.srcPinLabel, r.srcPin,
        r.resistanceMohm.toStringAsFixed(1), limitMohm, r.status, r.dstPin,
        r.dstPinLabel, r.dstConnId, '', '', //
      ]));
    }
    return buf.toString();
  }
